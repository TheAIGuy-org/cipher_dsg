...
class RealtimeAgent:
    ...
    def _process_bundle(self, bundle):
        ...
        self.approved_contents = []   # reset per bundle
        ...
        for plan_idx, plan in enumerate(plans, 1):
            ...
            try:
                ...
                self.errors_encountered += 1
                continue   # skip to next plan, don't break bundle
            ...
            while True:
                try:
                    ...
                    if choice == 'A':
                        ...
                        break   # ← just break inner while, continue to next plan

                    elif choice == 'Q':
                        print(f"\n⚠️  User requested quit")
                        self.running = False
                        # Still send any already-approved changes before quitting
                        break
            ...
            # If user quit, stop processing further plans in this bundle
            if not self.running:
                break

        # ── Send all approved changes at once ──────────────────────────────
        print(f"\n🎉 Bundle processing complete!")
        if self.approved_contents:
            print(f"\n   Sending {len(self.approved_contents)} approved change(s) to DOSSIER GENERATION MODULE")
            run_prompt(self.approved_contents)
            self.approved_contents = []
        else:
            print(f"\n   No changes approved — DOCX generation skipped")
